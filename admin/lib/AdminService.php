...
class AdminService {
    ...
    private static function acctCreateSmtp(): void {
        $email = $_POST['email'] ?? '';
        $pw = $_POST['password'] ?? '';
        if ($email) { if (!$pw) $pw = bin2hex(random_bytes(12)); maddy('maddy creds create ' . escapeshellarg($email), $pw); setFlash('Created SMTP credential: ' . $email); }
        header('Location: /accounts.php'); exit;
    }
    ...
    public static function getConnInfo(): array {
        ...
        if (is_readable($conf_file)) {
            ...
            if (preg_match_all('/^\s*(smtp|submission|imap)\s+([^\{\n]+)/m', $c, $mats, PREG_SET_ORDER)) {
                foreach ($mats as $m) {
                    $key = $m[1]; $parts = preg_split('/\s+/', trim($m[2]));
                    foreach ($parts as $p) {
    ...
    public static function getDnsData(): array {
        ...
        $serverIp = gethostbyname($hostname); if ($serverIp === $hostname) $serverIp = null;
